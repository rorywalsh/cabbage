...
instr    2    ; triggered by 'play/stop' button
 ...
 if giReady = 1 then                        ; i.e. if a file has been loaded
  ...
  rireturn
   ...
   a1   lposcil 0.2,1,0,ftlen(gitableL),gitableL
   a2   lposcil 0.2,1,0,ftlen(gitableL),gitableL
     outs    a1*aenv,a2*aenv                    ; send stereo signal to outputs
...
instr    3
 ...
 if giReady = 1 then                        ; i.e. if a file has been loaded
  ...
  iwfn        =    i(gkshape) + giwfn1 - 1
  if gichans==1 then                        ; if mono...
   a1    syncgrain    klevel*iamp, kdensity, iFrqRatio, kgrsize, (kprate*gkfreeze)/(kdensity*kgrsize), gitableL, iwfn, giolaps
     outs    a1*aenv,a1*aenv                    ; send mono audio to both outputs 
  elseif gichans==2 then                        ; otherwise, if stereo...
   a1    syncgrain    klevel*iamp, kdensity, iFrqRatio, kgrsize, (kprate*gkfreeze)/(kdensity*kgrsize), gitableL, iwfn, giolaps
   a2    syncgrain    klevel*iamp, kdensity, iFrqRatio, kgrsize, (kprate*gkfreeze)/(kdensity*kgrsize), gitableR, iwfn, giolaps
     outs    a1*aenv,a2*aenv                    ; send stereo signal to outputs
  endif
  rireturn
 endif
