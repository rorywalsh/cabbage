...
//==============================================================================
nlohmann::ordered_json CabbagePluginProcessor::addPluginPreset(String presetName,  const String& fileName, bool remove)
{    
...
void CabbagePluginProcessor::setCabbageParameter(String& channel, float value, ValueTree& wData)
{
    if(pollingChannels() == 0){
        MessageManager::callAsync ([ wData, channel, value](){
            ...
            {
                if(CabbageWidgetData::getStringProp(wData, CabbageIdentifierIds::channeltype) != CabbageIdentifierIds::stringchannel)
...
void CabbagePluginProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
	...
	//grab the current plugin state so we can reinstate it if Csound is recompiled due to channel/SR changes..
    //state in this instance is the state saved using the state opcodes
	if (getCsound())
	{
		auto** p = (CabbagePersistentData**)getCsound()->QueryGlobalVariable("cabbageData");
