...
FilterGraphDocumentWindow* IOConfigurationWindow::getGraphDocumentWindow() const
{
	auto& desktop = Desktop::getInstance();
	
	for (int i = desktop.getNumComponents(); --i >= 0;)
		if (auto* mainWindow = dynamic_cast<FilterGraphDocumentWindow*> (desktop.getComponent(i)))
	        return mainWindow;
...
GraphDocumentComponent* IOConfigurationWindow::getGraphEditor() const
{
	...
	return getGraphDocumentWindow()->getOwner()->getGraphComponent();
    //return nullptr;
}
