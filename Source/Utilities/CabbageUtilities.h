...
public:
	...
	static void writePluginFileToDisk (File fc, File csdFile, File VSTData, String fileExtension, String pluginId)
	{
		...
		if (!VSTData.copyFileTo (dll))
			showMessage ("Can't copy plugin lib, is it currently in use?");
		...
		if ((SystemStats::getOperatingSystemType() & SystemStats::MacOSX) != 0)
		{
			...
			if (bin.moveFileTo (pluginBinary) == false)
				showMessage ("Could not copy library binary file. Make sure the two Cabbage .vst files are located in the Cabbage.app folder");
	...
	//==============================================================================
	static int setUniquePluginId (File binFile, File csdFile, String pluginId)
	{
	...
	static long cabbageFindPluginId (unsigned char* buf, size_t len, const char* s)
	{
		long i, j;
